...
def divide_music(input_song='input.wav'):
    ...
    for part in range(length_song / ss):
        file_path('temp/' + input_song.split('.')[0] + '/')
        temp_path = 'temp/' + input_song.split('.')[0] + '/'
        song[part * ss:ss * (part + 1)].export(temp_path + input_song.split(".")[0] + '~~~' + str(part) + 'wav', 'wav')
    return 0


def conbination_all_pieces(path_to_file='temp_path'):
    for parent, dirnames, filenames in os.walk(path_to_file):
        total_num = len(filenames)
        for filename in filenames:
            wav_file = os.path.join(parent, filename)
        name = os.path.basename(wav_file).split('.')[0].split('~~~')[0]
    song = AudioSegment.from_wav(path_to_file + name + '~~~0.wav')
    os.remove(path_to_file + name + '~~~0.wav')

    for part in range(total_num - 1):
        song = song + AudioSegment.from_wav(path_to_file + name + str(part + 1) + '.wav')
        os.remove(path_to_file + name + str(part + 1) + '.wav')
    song.export(path_to_file + name + 'conbination.wav', 'wav')
    return 0
...
def convert_from_mp3(get_musicfile):
    out_get_musicfile = get_musicfile.split.split('.')[0] + '.wav'
    song = AudioSegment.from_mp3(get_musicfile)
    song.export(out_get_musicfile, 'wav')
    return out_get_musicfile
...
def main(wav_file, parm):
    get_musicfile = wav_file

    if get_musicfile.split('.')[1] == "wav":
        pass

    elif get_musicfile.split('.')[1] == "mp3":
        out_get_musicfile = convert_from_mp3(get_musicfile)
        get_musicfile = out_get_musicfile

    elif get_musicfile.split('.')[1] == "mp4":
        cmd = "ffmpeg -i" + get_musicfile + "-y " + get_musicfile.split('.')[0] + "_output.wav"
        os.system(cmd)
        get_musicfile = get_musicfile.split('.')[0] + "_output.wav"

    else:
        print 'error'
    voc_output_file = out_get_musicfile.split('.')[0] + "_vocal.wav"
    mus_output_file = out_get_musicfile.split('.')[0] + "_accompaniment_music.wav"

    pitch_output_file = "pitches.txt"

    # Some more optional options:
    displayEvolution = False
    verbose = True

    nbiter = parm.get('nbiter')
    windowSize = parm.get('windowSize')
    fourierSize = parm.get('fourierSize')
    hopsize = parm.get('hopesize')
    R = parm.get('R')
    ...
    windowSizeInSamples = np.round(windowSize * fs)
    hopsize = np.round(hopsize * fs)
    NFT = fourierSize
    niter = nbiter
    ...
    if melody is None:
        ...
        HGAMMA, HPHI, HF0, HM, WM, recoError1 = SIMM.SIMM(
            ...
            verbose=verbose, displayEvolution=displayEvolution)
        ...
        indexBestPath = viterbiTrackingArray( \
            ...
            np.log(transitionMatrixF0), verbose=verbose)
        ...
        melodyFromFile = np.loadtxt(melody)
    ...
    outputFileName = voc_output_file[:-4] + '_VUIMM.wav'
    ...
    outputFileName = mus_output_file[:-4] + '_VUIMM.wav'
